...
export const RunningAppsWidget = GObject.registerClass(
  class RunningAppsWidget extends QuickToggle {
    ...
    _setOpenedSubMenu(submenu) {
      const current = this.menu._openedSubMenu;
       ...
       */
      if (!current) {
        this.menu._openedSubMenu = submenu;
       ...
       */
      if (this._isDescendantMenu(submenu, current)) {
        return;
      }
       ...
       */
      if (this._isDescendantMenu(current, submenu)) {
        try {
          current.close(true);
        } catch {
    ...
    _isDescendantMenu(menu, ancestor) {
    ...
    addIndicator(indicator) {
      if (this._items.has(indicator.uniqueId)) {
        return;
      }

      const item = new RunningAppItem(indicator);
