...
extends Signals.EventEmitter {
        ...
        this._nameId = Gio.bus_own_name(
            ...
            connection => {
                ...
                try {
                    ...
                    logError(
                        e,
                        'Unable to export StatusNotifierWatcher'
                    );
    ...
    _unexportWatcher() {
        ...
        try {
            ...
            logError(
                e,
                'Unable to unexport StatusNotifierWatcher'
            );
        }
    ...
    _onNameAcquired() {
        console.log(
            ...
            'StatusNotifierWatcher acquired'
        );
    ...
    async RegisterStatusNotifierItemAsync(
        params,
        invocation
    ) {
        ...
        try {
            ...
            if (!busName) {
                throw new Error(
                    `Unable to resolve StatusNotifierItem ${service}`
                );
            }
            ...
            invocation.return_dbus_error(
                'org.gnome.gjs.StatusNotifierError',
                e.message
            );
        }
    }
    ...
    _register(
        ...
        try {
            ...
            logError(
                e,
                `Could not create StatusNotifierItem ${key}`
            );
    ...
    _remove(item) {
        if (!this._items.has(item.uniqueId))
            return;
    ...
    _emitItemsChanged() {
        ...
        try {
            this._dbusImpl.emit_property_changed(
                ...
                new GLib.Variant(
                    'as',
                    this.RegisteredStatusNotifierItems
                )
            );
        } catch (e) {
            logError(e);
    ...
    async _discoverExistingItems() {
        ...
        try {
            ...
            logError(
                e,
                'Unable to enumerate session bus'
            );
        ...
        for (const name of names) {
            ...
            const owner =
                await this._resolveBusName(name);
